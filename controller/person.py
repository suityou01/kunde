...
@aiohttp_jinja2.template('person_dashboard.jinja2')
async def person_dashboard(request):
    return
...
async def on_startup(userapi):
    ...
    print("person_controller sub_app startup")
    
async def on_shutdown(userapi):
    #log.info("org sub_app shutdown")
    print("person_controller sub_app shutdown")

person_controller = web.Application()
log = logging.getLogger(__name__)

person_controller.on_shutdown.append(on_shutdown)
person_controller.on_startup.append(on_startup)
person_controller.add_routes([web.get('/', person_dashboard, name='person_dashboard')])
