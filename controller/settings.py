...
@aiohttp_jinja2.template('settings_dashboard.jinja2')
...
async def on_startup(userapi):
    ...
    print("settings_controller sub_app startup")
    
async def on_shutdown(userapi):
    #log.info("org sub_app shutdown")
    print("settings_controller sub_app shutdown")

settings_controller = web.Application()
log = logging.getLogger(__name__)

settings_controller.on_shutdown.append(on_shutdown)
settings_controller.on_startup.append(on_startup)
settings_controller.add_routes([web.get('/', settings_dashboard, name='settings_dashboard')])
