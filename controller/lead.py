...
@aiohttp_jinja2.template('lead_dashboard.jinja2')
async def lead_dashboard(request):
    return
...
async def on_startup(userapi):
    ...
    print("lead_controller sub_app startup")
    
async def on_shutdown(userapi):
    #log.info("org sub_app shutdown")
    print("lead_controller sub_app shutdown")

lead_controller = web.Application()
log = logging.getLogger(__name__)

lead_controller.on_shutdown.append(on_shutdown)
lead_controller.on_startup.append(on_startup)
lead_controller.add_routes([web.get('/', lead_dashboard, name='lead_dashboard')])
