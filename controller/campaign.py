...
@aiohttp_jinja2.template('campaign_dashboard.jinja2')
async def campaign_dashboard(request):
    return
...
async def on_startup(userapi):
    ...
    print("campaign_controller sub_app startup")
    
async def on_shutdown(userapi):
    #log.info("org sub_app shutdown")
    print("campgaign_controller sub_app shutdown")

campaign_controller = web.Application()
log = logging.getLogger(__name__)

campaign_controller.on_shutdown.append(on_shutdown)
campaign_controller.on_startup.append(on_startup)
campaign_controller.add_routes([web.get('/', campaign_dashboard, name='campaign_dashboard')])
