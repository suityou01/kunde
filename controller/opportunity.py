...
@aiohttp_jinja2.template('opportunity_dashboard.jinja2')
async def opportunity_dashboard(request):
    return
...
async def on_startup(userapi):
    ...
    print("opportunity_controller sub_app startup")
    
async def on_shutdown(userapi):
    #log.info("org sub_app shutdown")
    print("opportunity_controller sub_app shutdown")

opportunity_controller = web.Application()
log = logging.getLogger(__name__)

opportunity_controller.on_shutdown.append(on_shutdown)
opportunity_controller.on_startup.append(on_startup)
opportunity_controller.add_routes([web.get('/', opportunity_dashboard, name='opportunity_dashboard')])
