...
@aiohttp_jinja2.template('mail_dashboard.jinja2')
async def mail_dashboard(request):
    return
...
async def on_startup(userapi):
    ...
    print("mail_controller sub_app startup")
    
async def on_shutdown(userapi):
    #log.info("org sub_app shutdown")
    print("mail_controller sub_app shutdown")

mail_controller = web.Application()
log = logging.getLogger(__name__)

mail_controller.on_shutdown.append(on_shutdown)
mail_controller.on_startup.append(on_startup)
mail_controller.add_routes([web.get('/', mail_dashboard, name='mail_dashboard')])
